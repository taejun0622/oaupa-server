...
class AirtableProvider(OAuthProviderBase):
    ...
    async def refresh_token(self, refresh_token: str) -> TokenResponse:
        return await refresh_token_standard(
            self.token_url, self.client_id, self.client_secret, refresh_token, use_basic_auth=True
        )
    ...
    async def get_account_info(self, access_token: str) -> AccountInfo | None:
        async with httpx.AsyncClient() as client:
            ...
            if resp.status_code != 200:
                return None
            data = resp.json()
            return AccountInfo(account_id=data.get("id", ""), email=data.get("email"))
